...
class OR1KTargetLayout: public TargetLayout<ELFTy> {
public:
    enum OR1KSectionOrder{
        ORDER_VECTORS = 1
    };
    ...
    AtomSection<ELFTy> *createSection(
        ...
        return TargetLayout<ELFTy>::createSection(
                    name, contentType, contentPermissions, sectionOrder);
    }

    typename TargetLayout<ELFTy>::SectionOrder getSectionOrder(
          StringRef name, int32_t contentType, int32_t contentPermissions) override {
        if (name == ".vectors") return ORDER_VECTORS;
        return TargetLayout<ELFTy>::getSectionOrder(
                    name, contentType, contentPermissions);
    }

    typename TargetLayout<ELFTy>::SegmentType getSegmentType(
          Section<ELFTy> *section) const override {
        if (section->order() == ORDER_VECTORS) return llvm::ELF::PT_LOAD;
    ...
    uint64_t getGOTSymAddr() {
        ...
        if (*_gotSymAtom) return (*_gotSymAtom)->_virtualAddr;
        return 0;
    }
...
template <typename ELFTy>
class OR1KTargetHandler final: public TargetHandler {
private:
...
public:
    OR1KTargetHandler(OR1KLinkingContext &ctx):
        _ctx(ctx),
        _targetLayout(new OR1KTargetLayout<ELFTy>(ctx)),
        _relocationHandler(createOR1KRelocationHandler<ELFTy>(_ctx, *_targetLayout))
            {}
...
std::unique_ptr<TargetHandler> createOR1KTargetHandler(
