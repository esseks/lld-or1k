...
class OR1KVectorsSection: public AtomSection<ELFTy> {
public:
    OR1KVectorsSection(ELFLinkingContext const& ctx):
        AtomSection<ELFTy>(
            ctx, ".vectors", DefinedAtom::typeCode, DefinedAtom::permR_X,
    ...
    void assignFileOffsets(uint64_t offset) override {
        assert(!this->atoms().empty()
            && ".vectors section should not be empty");
        ...
        assert(offset < paddingAtom->size()
            && "ELF header too big");
