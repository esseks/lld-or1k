...
public:
  OR1KRelocationHandler(OR1KLinkingContext &ctx,
                        OR1KTargetLayout<ELFTy> &layout)
      : _ctx(ctx), _layout(layout) {}

  std::error_code applyRelocation(lld::elf::ELFWriter &writer,
                                  llvm::FileOutputBuffer &buf,
                                  lld::AtomLayout const &atom,
                                  lld::Reference const &ref) const override;
...
std::error_code OR1KRelocationHandler<ELFTy>::applyRelocation(
  ...
  if (ref.kindValue() == R_OR1K_GOTPC_HI16 ||
      ref.kindValue() == R_OR1K_GOTPC_HI16) {
...
createOR1KRelocationHandler(OR1KLinkingContext &ctx,
                            OR1KTargetLayout<OR1KBEType> &layout) {
...
std::unique_ptr<TargetRelocationHandler>
createOR1KRelocationHandler(OR1KLinkingContext &ctx,
                            OR1KTargetLayout<OR1KLEType> &layout) {
