...
#define DEF_ATOM(name, base, ...) \
    static const uint8_t name##_content[] = { \
        __VA_ARGS__ \
    }; \
...
class RelocationPass: public lld::Pass {
public:
    RelocationPass(const lld::ELFLinkingContext &ctx):
        _file(ctx), _ctx(ctx) {}
...
template <typename Derived>
void RelocationPass<Derived>::perform(std::unique_ptr<lld::SimpleFile> &mergedFile) {
 ...
 */
template <typename Derived>
void RelocationPass<Derived>::handleReference(
    const lld::DefinedAtom &atom, const lld::Reference &ref
) {
    if (ref.kindNamespace() != lld::Reference::KindNamespace::ELF) return;
...
template <typename Derived>
lld::elf::PLTAtom const* RelocationPass<Derived>::getIFUNCPLTEntry(lld::DefinedAtom const* da) { //TODO split static/dinamico
    auto plt = _pltMap.find(da);
    if (plt != _pltMap.end()) return plt->second;
...
template <typename Derived>
lld::elf::GOTAtom const* RelocationPass<Derived>::getGOT(const lld::DefinedAtom *da) {
    auto got = _gotMap.find(da);
    if (got != _gotMap.end()) return got->second;
...
template <typename Derived>
lld::elf::PLTAtom const* RelocationPass<Derived>::getPLTEntry(lld::Atom const* a) {
...
template <typename Derived>
lld::elf::ObjectAtom const* RelocationPass<Derived>::getObjectEntry(lld::SharedLibraryAtom const* a) {
    auto obj = _objectMap.find(a);
    if (obj != _objectMap.end()) return obj->second;
...
template <typename Derived>
lld::elf::GOTAtom const* RelocationPass<Derived>::getSharedGOT(lld::Atom const* a) {
...
static inline
bool isSharedOrUndefined(lld::Atom const* atom) {
...
void StaticRelocationPass::handleGOT(lld::Reference const& ref) {
    ...
    atom = llvm::dyn_cast_or_null<const lld::DefinedAtom>(ref.target());
...
void StaticRelocationPass::handlePLT(lld::Reference const& ref) {
    ...
    const_cast<lld::Reference &>(ref).setKindValue(llvm::ELF::R_OR1K_32_PCREL); //FIXME
...
void DynamicRelocationPass::handlePlain(lld::Reference const& ref) {
    ...
    atom = llvm::dyn_cast_or_null<const lld::SharedLibraryAtom>(ref.target());
...
void DynamicRelocationPass::handleGOT(lld::Reference const& ref) {
    lld::DefinedAtom const*
    atom = dyn_cast_or_null<const lld::DefinedAtom>(ref.target());
